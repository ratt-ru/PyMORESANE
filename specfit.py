...
def get_spec(fitsname,spi=True,spc=False,sp2=False,start=0,end=0,
    ...
    imdata[freq_axis] = np.arange(start,end,1)
    ...
    mfs = data[imdata].sum(0)/(end-start) # average map over frequency
    thresh = mad(mfs,sigma_level) * math.sqrt(end-start) # Noise in each channel ~sqrt(bandwith/channel width)

    hdr['CRVAL%d'%abs(freq_axis-naxis)] = freq0# Update fits header for MFS map header
    # Save mfs image
    pyfits.writeto(fitsname.replace('.fits','-MFS.fits'),data.sum(freq_axis)/(end-start),hdr,clobber=True)
